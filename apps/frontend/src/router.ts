...
router.get("/cdn/css", (_: any, res: Response) => {
  res.header("Content-Type", "text/css").send(
    fs.readFileSync(__dirname + "/routes/cdn/css/global.css", {
      encoding: "utf-8",
    }) +
...
router.get("/cdn/js", (_: any, res: Response) => {
  res.header("Content-Type", "text/javascript").send(
    fs.readFileSync(__dirname + "/routes/cdn/js/reva-ui.min.js", {
      encoding: "utf-8",
    }) +
...
router.get("*", (_: any, res: Response) => {
  res
    ...
    .send(
      '<html style="background-color: black;"><head><title>404 not found</title></head><body><h1 style="color: white;">404 not found</h1></body</html>',
    );
});
